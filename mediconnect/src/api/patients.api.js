...
// ── Mon profil patient (PATIENT connecté) ────────────────────────────────────
export const getMonProfil = async () => {
  const { data } = await client.get("/api/patients/me");
...
// ── RGPD : demande de suppression de compte (PATIENT connecté) ────────────────
export const demanderSuppressionCompte = async () => {
  const { data } = await client.post("/api/patients/moi/demande-suppression");
  return data; // { success, message }
};

// ── RGPD : export des données personnelles (PATIENT connecté) ─────────────────
export const exporterMesDonnees = async () => {
  const { data } = await client.get("/api/patients/moi/mes-donnees");
  return data; // { success, data: { profil, exporteLe, ... } }
};